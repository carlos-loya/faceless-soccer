...
| `docs/HARNESS-PORTABILITY.md` | **Cross-harness guide** — how the skills + slash commands run outside Claude Code (OpenCode first). Skills are read natively by OpenCode from `.claude/skills/`; commands are translated to `.opencode/command/`; `CLAUDE.md` is bridged to `AGENTS.md`. |
| `pipeline/sync_harness.py` | **Harness sync generator** — keeps `.claude/` as the single source of truth and regenerates the harness-specific copies: translates `.claude/commands/*.md` → `.opencode/command/*.md` (drops `allowed-tools`/`argument-hint`, qualifies `model` to `provider/model`), mirrors skills+commands → `.agents/skills/` + `.agents/commands/` (verbatim, for **Antigravity** which reads `.agents/` not `.claude/`; skips `find-skills` whose real files already live there), bridges `AGENTS.md`→`CLAUDE.md`, sanity-checks skills. `sync` (write) / `check` (drift, exit 1 if stale). Pure stdlib. Re-run after editing any command or adding a skill. |
| `.opencode/command/`, `.agents/skills/`, `.agents/commands/`, `AGENTS.md` | **Generated, do not hand-edit** — OpenCode-format command copies, the Antigravity skill+command mirror, and the instructions bridge, produced by `sync_harness.py`. Edit the `.claude/` source, then re-run sync. (Exception: `.agents/skills/find-skills/` is the REAL find-skills, symlinked back from `.claude/`.) |
...
- ⚠️ **Postiz (fallback only):** OAuth app must be published to **Production** or the YT token expires every 7 days. Default path is `upload_youtube.py` (YT) + `post-social` skill (TikTok/IG).
