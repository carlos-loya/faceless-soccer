...
| Voiceover (production) | **ElevenLabs** | `elevenlabs/skills@text-to-speech`; used only on `TTV_PRODUCTION=1` runs |
| Voiceover (draft, DEFAULT) | **VoiceBox** (local voice-clone) | owner's own cloned voice "Carlos" via the local VoiceBox desktop app (REST `POST /generate`, free, no rights issue). Default draft voice; **auto-falls back to Piper** if the app isn't running; `TTV_PIPER=1` forces Piper |
...
| `pipeline/dashboard/` | **Mission Control dashboard** — a local SaaS-style control room (`bash pipeline/dashboard/run.sh` → http://localhost:8770; stdlib-only Python, binds 127.0.0.1, single self-contained `index.html`). Tabs: **Overview** (channel KPIs + pipeline funnel + auto status + fixtures + VoiceBox/comments indicators), **Pipeline** (card board; status from one server-computed `stage` where **published is authoritative from `post-log.jsonl`** regardless of local files — fixes live videos showing under "Needs work"; **draft has a Carlos/Piper voice chooser**; TikTok/IG handoff via `post_packet.py`), **Analytics** (`youtube_analytics.py collect`), **Comments** (`youtube_comments.py fetch`, read-only reply handoff), **Automation** (edit `pipeline/auto/config.json`, run history, ESPN result gate, gated "Run now"), **Schedule**, **Ideation**, **Entities**. `server.py` runs scripts as background jobs with a live transmission log and serves storyboards/renders (MP4 range); lighter tabs read raw repo JSON via the static route. **The server CAN now invoke Claude** (owner opt-in, 2026-07-09): brain buttons fire `claude -p /find-topics` / `/daily` and Automation's "Run now" shells `run_match_pipeline.sh` — all clearly tagged **spends tokens** and confirm-gated. Every side-effecting/token-spending action confirms first; localhost-only. |
...
- **ElevenLabs credits = money — ALWAYS draft first.** `make_video.sh` defaults to a FREE local draft; only run `TTV_PRODUCTION=1` (real ElevenLabs VO) on a spec whose visuals/captions/pacing have been reviewed and approved.
- ✅ **Draft voice = VoiceBox "Carlos" (2026-07-08).** The default draft VO is now the owner's own cloned voice via the local VoiceBox app (`generate_assets.py` `voicebox_tts` → REST `POST /generate` → WAV → mp3), replacing the robotic Piper voice for previews — free, no credits, no commercial-rights issue. ⚠️ VoiceBox is a **Windows desktop app that must be running**; when it's unreachable the draft **auto-falls back to Piper** (so headless/auto-cron renders never break). `TTV_PIPER=1` forces Piper. **Production is unchanged — still ElevenLabs** on `TTV_PRODUCTION=1`. VoiceBox gives no word-level timestamps, so draft karaoke uses the same length-weighted `split_words` approximation Piper did. Config: `TTV_VOICEBOX_PROFILE`/`TTV_VOICEBOX_URL`/`TTV_VOICEBOX_GENDIR` (URL + gendir are machine-specific). Caption/layout/image fixes are free to iterate in draft. If only the caption text (not audio) is wrong on an already-produced video, patch the words in `out/assets/<stem>/props.json` and re-run `prepare.mjs` + Remotion render — never re-run TTS just to fix on-screen text.
