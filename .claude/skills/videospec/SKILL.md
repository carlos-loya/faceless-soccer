...
5. **SUBSCRIBE capture (REQUIRED — this is the monetization gate).** The channel must convert viewers to subs (500 → fan-funding, 1,000 → ad revenue; currently 3, with 0% conversion). Every spec MUST include a subscribe beat in the **final content scene**, tied to a **series identity** (e.g. "your daily World Cup stories", "the road to 2026, every day"):
   - put the nudge in that scene's **`voiceover`** (it gets voiced — `cta` does NOT render or get spoken, so never rely on it alone) AND
   - burn a short version into that scene's **`on_screen_text`** (e.g. `SUBSCRIBE — DAILY WORLD CUP`) for the sound-off majority.
   - also set `cta` to name the hook (used in captions/metadata).
   ~1 second, confident, never cringe, never an emoji. Note the end card then voices the `comment_bait` after — so keep the last scene's VO as the SUBSCRIBE line, not a second debate question.
...
6. **The ending is for the LOOP, not a dead card.** Don't close on a generic "subscribe" crowd card — it converts nobody and breaks the replay. Return the FINAL visual to the hook (same star/shot) so an auto-replay feels continuous, and weave the subscribe into that beat. Set `retention_mechanic: seamless_loop` and actually build the loop back to scene 1.
